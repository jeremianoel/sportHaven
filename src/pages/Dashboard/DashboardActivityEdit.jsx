...
const DashboardActivityEdit = () => {
    ...
    useEffect(() => {
    if (detail && detail.id) {
        setSportCategoryId(detail.sport_category_id);
        setCityId(detail.city_id);
        setTitle(detail.title);
        setDescription(detail.description);
        setSlot(detail.slot);
        setPrice(detail.price);
        setAddress(detail.address);
        setActivityDate(detail.activity_date);
        setStartTime(detail.start_time?.slice(0, 5));
        setEndTime(detail.end_time?.slice(0, 5)); 
        setMapURL(detail.map_url || 'https://maps.app.goo.gl/h1AV4bfB2cojJMxK7');
    }
    }, [detail]);
    ...
    const editActivity = async (e) => {
        ...
        try {
           ...
           const res = await axios.post(`https://sport-reservation-api-bootcamp.do.dibimbing.id/api/v1/sport-activities/update/${id}`,payload,
                {
                    headers: {
    ...
    return (
        ...
        <div className="flex flex-col h-auto bg-white shadow-md rounded-md justify-center items-center w-full m-3 gap-5">
            <form onSubmit={(e) => editActivity(e)} className='flex-col flex items-center text-sm gap-2 py-15'>
              <h1 className='font-bold text-3xl mb-3 text-gray-900 text-start w-full'>Edit Activity</h1>
...
export default DashboardActivityEdit
